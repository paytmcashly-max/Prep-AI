...
const authenticatedRequest = async ({
  ...
  const { cleanup, signal } = createTimeoutSignal(options.timeoutMs);

  try {
    const token = await getAuthToken();

    if (method !== "GET") {
      logDevelopmentRequest(path, logBody ?? body);
    }
    ...
    if (payload === null && options.allowNull) {
      return null;
    }
...
export const getAuthenticatedJson = async (path, options = {}) =>
  authenticatedRequest({
    method: "GET",
    options,
    path
...
export const getUsageStatus = () => getAuthenticatedJson("/api/usage/status");
